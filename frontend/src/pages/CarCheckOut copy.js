...
function CarCheckOut({ bookingDetails }) {
  ...
  useEffect(() => {
    fetch("http://144.24.107.76/car-service-redis/cars"+`/${id}`)
  ...
  useEffect(() => {
    fetch(`http://144.24.107.76/user-service-redis/users/${getUserId}`)
      .then((response) => response.json())
      .then((data) => {
        // console.log("from http://146.56.171.43:8081 ", id);
        setUserData(data);
  ...
  const handleCheckout = async () => {
    ...
    const response = await fetch("http://144.24.107.76/order-service/create-order", {
      method: 'POST',
      headers: {
